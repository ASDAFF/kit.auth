...
class CompanyJoin extends CBitrixComponent implements Controllerable
{
    ...
    function executeComponent()
    {
        ...
        $currentCompanies = [];
        $obStaff = Sotbit\Auth\Internals\StaffTable::getList( [
            'filter' => [
                'USER_ID' => $USER->GetID(),
                'COMPANY_STATUS' => 'A'
            ],
        ...
        $obCompany = Sotbit\Auth\Internals\CompanyTable::getList($parametersCompany)->fetchAll();
        ...
        $obValuesInn = Sotbit\Auth\Internals\CompanyPropsValueTable::getList($parametersValue)->fetchAll();
    ...
    public function joiningCompanyAction()
    {
        if ($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["JOIN_COMPANY_ID"] <> '' && !empty($_REQUEST["JOIN_COMPANY_ID"])) {
           global $USER;
            ...
            return [
              'error' => true,
              'errorMessage' => GetMessage("SOTBIT_AUTH_COMPANY_JOIN_ERROR_MESSAGE_COMPANY_NOT_CHOOSE"),
            ];
        }
    }
