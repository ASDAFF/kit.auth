...
Loader::registerAutoLoadClasses('sotbit.auth',
    array(
...
class SotbitAuth
{
    const idModule = 'sotbit.auth';
    const SEND_FORGOT_NEW_PASSWORD_EVENT = 'SOTBIT_AUTH_SUCCESS_CHANGE_PASSWORD';
    const SEND_NEW_USER_PASSWORD_EVENT = 'SOTBIT_AUTH_NEW_USER_PASSWORD';
    const SOTBIT_AUTH_SEND_EVENT = 'SOTBIT_AUTH_SEND';
...
class DataManagerEx_Auth extends Bitrix\Main\Entity\DataManager
{
    ...
    {
        if (!SotbitAuth::getDemo()) return new Bitrix\Main\ORM\Query\Result(parent::query(), new \Bitrix\Main\DB\ArrayResult(array())); else return parent::getList($_501887963);
    }

    public static function getById($_2018094269 = "")
    {
        if (!SotbitAuth::getDemo()) return new \CDBResult; else return parent::getById($_2018094269);
    }

    public static function add(array $_126901937 = array())
    {
        if (!SotbitAuth::getDemo()) return new \Bitrix\Main\Entity\AddResult(); else return parent::add($_126901937);
    }

    public static function update($_2018094269 = "", array $_126901937 = array())
    {
        if (!SotbitAuth::getDemo()) return new \Bitrix\Main\Entity\UpdateResult(); else return parent::update($_2018094269, $_126901937);
    }
}