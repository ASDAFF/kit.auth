	...
	public function sendPassword($Site = '')
	{
		...
		if( $idEvent )
		{
			\CEventLog::Add( 
					array(
							'SEVERITY' => 'WARNING',
							'AUDIT_TYPE_ID' => 'SOTBIT_AUTH_SUCCESS_SEND_PASSWORD_TO_NEW_USER_IN_ADMIN',
							'MODULE_ID' => self::idModule,
							'DESCRIPTION' => Loc::getMessage( 'SOTBIT_AUTH_SUCCESS_SEND_PASSWORD_TO_NEW_USER_IN_ADMIN', 
									array(
		...
		else
		{
			\CEventLog::Add( 
					array(
							...
							'MODULE_ID' => self::idModule,
							'DESCRIPTION' => Loc::getMessage( 'SOTBIT_AUTH_ERROR_SEND_PASSWORD_TO_NEW_USER_IN_ADMIN', array(
									'#USER_LOGIN#' => $this->getField( 'LOGIN' ) 
							) ),
							'ITEM_ID' => self::idModule 
					) );
