...
class EventHandlers extends \SotbitAuth
{
						...
						if( !$Mail->User()->getError() )
						{
							\CEventLog::Add( 
									array(
											...
											'MODULE_ID' => $SotbitAuth::idModule,
											'DESCRIPTION' => Loc::getMessage( "SOTBIT_AUTH_SUCCESS_CHANGE_PASSWORD", 
													array(
															'#USER_ID#' => $Mail->User()->getField( 'ID' ),
															'#USER_LOGIN#' => $Mail->User()->getField( 'LOGIN' ) 
													) ),
											'ITEM_ID' => $SotbitAuth::idModule 
									) );
									$result = \Bitrix\Main\Mail\Event::send( 
											array(
													'EVENT_NAME' => $SotbitAuth::SEND_FORGOT_NEW_PASSWORD_EVENT,
							...
							if( $result->isSuccess() )
							{
								\CEventLog::Add( 
										array(
												...
												'MODULE_ID' => $SotbitAuth::idModule,
												'DESCRIPTION' => Loc::getMessage( "SOTBIT_AUTH_SUCCESS_SEND_PASSWORD", 
														array(
																'#USER_ID#' => $Mail->User()->getField( 'ID' ),
																'#USER_LOGIN#' => $Mail->User()->getField( 'LOGIN' ),
							...
							else
							{
								\CEventLog::Add( 
										array(
												...
												'MODULE_ID' => $SotbitAuth::idModule,
												'DESCRIPTION' => Loc::getMessage( "SOTBIT_AUTH_ERROR_SEND_PASSWORD", 
														array(
																'#USER_ID#' => $Mail->User()->getField( 'ID' ),
																'#USER_LOGIN#' => $Mail->User()->getField( 'LOGIN' ) 
														) ),
								...
								$Mail->User()->setError( Loc::getMessage( "SOTBIT_AUTH_ERROR" ) );
							}
						}
					}
					else
					{
						$Mail->User()->setError( Loc::getMessage( "SOTBIT_AUTH_USER_NOT_FOUND" ) );
					}
				}
				else
				{
					$Mail->User()->setError( Loc::getMessage( "SOTBIT_AUTH_ERROR_WRONG_EMAIL" ) );
				}
				if( $Mail->User()->getError() )
				{
	...
	public function OnBeforeEventSendHandler($Fields, $Template)
	{
		...
		if( \Bitrix\Main\Loader::includeModule( "sotbit.auth" ) )
		{
			$Events = Option::get("sotbit.auth", 'AUTH_FROM_EMAIL_EVENTS','a:0:{}',$Template['LID']);
			...
			if( (Option::get( self::idModule, 'AUTH_FROM_EMAIL', 'Y', $Template['LID'] ) == 'Y' /*|| Option::get( self::idModule, 'AUTH_STATISTICS', 'Y', $Template['LID'] ) == 'Y'*/) && Option::get( self::idModule, 'AUTH_FROM_EMAIL_DOMAIN', '', $Template['LID'] ) && is_array($Events) && in_array($Template['EVENT_NAME'],$Events))
			{
				...
				$event = new \Bitrix\Main\Event('sotbit.auth', 'OnBeforeSendMail', array(
						'MAIL' => $Mail
				));
				$event->send();
				...
				if($Mail->getField('NEW_TEXT'))
				{
					...
					$result = \Bitrix\Main\Mail\Event::send(array(
						"EVENT_NAME" => self::SOTBIT_AUTH_SEND_EVENT,
