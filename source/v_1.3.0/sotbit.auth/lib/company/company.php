 ...
 *
 */
class Company extends \SotbitAuth
{
    ...
    public function addCompany($companyFields, $personType, $userID = '')
    {
        ...
        if(is_array($companyFields) && !empty($companyFields)){
            ...
            $companyName = $companyFields[Option::get(\SotbitAuth::idModule, 'COMPANY_PROPS_NAME_FIELD_' . $personType, '', $this->getSite())];
            ...
            if ($resultAddCompany->isSuccess())
            {
                ...
                $rsEvents = GetModuleEvents(\SotbitAuth::idModule, "OnAfterAddCompany");
    ...
    public function getHash($companyFields, $personType) {
        $validatedFields = unserialize(Option::get(\SotbitAuth::idModule, 'GROUP_FIELDS_FOR_CHECK_'.$personType, '', $this->getSite()));
    ...
    public function confirmCompany($id)
    {
        ...
        $rsEvents = GetModuleEvents(\SotbitAuth::idModule, "OnBeforeCompanyModerate");
        while ($arEvent = $rsEvents->Fetch())
        {
        ...
        if($canModerate){
            ...
            if(!$rs->isSuccess())
            {
                ...
                $rsEvents = GetModuleEvents(\SotbitAuth::idModule, "OnAfterCompanyModerate");
                while ($arEvent = $rsEvents->Fetch())
                {
    ...
    public function unconfirmCompany($id)
    {
        ...
        if($confirmCompany["ACTIVE"] == "Y"){
            $canRejected = true;
            $rsEvents = GetModuleEvents(\SotbitAuth::idModule, "OnBeforeCompanyRejection");
            while ($arEvent = $rsEvents->Fetch())
            {
            ...
            $rsEvents = GetModuleEvents(\SotbitAuth::idModule, "OnBeforeCompanyChangesRejection");
            while ($arEvent = $rsEvents->Fetch())
            {
                if (ExecuteModuleEvent($arEvent, $confirmCompany["FIELDS"]["ORDER_FIELDS"], $confirmCompany["COMPANY_ID"]) === false)
                {
    ...
    public function getCompanyOrders()
    {
        ...
        $uniquePropsCode = unserialize(Option::get(\SotbitAuth::idModule, "GROUP_FIELDS_FOR_CHECK_2", "", $this->getSite()));
