...
class KitProfileAdd extends CBitrixComponent implements Controllerable
{
    ...
    {
        return !(!Loader::includeModule('sale') || !Loader::includeModule('kit.b2bcabinet') || !Loader::includeModule('kit.auth'));
    }
...
//		}
            ...
            if (Loader::includeModule('kit.auth')) {
                ...
                if ($this->companyId) {
                    ...
                    if ($issetCompany) {
                        ...
                        //company add
                        if(Option::get(KitAuth::idModule, 'CONFIRM_BUYER', 'N', SITE_ID) === 'Y'){
    ...
    public function checkFieldsAction()
    {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST)) {
            ...
            $checkPropsGroup = unserialize(Option::get(KitAuth::idModule, "COMPANY_PROPS_GROUP_ID_" . $request->get("PERSON_TYPE"), "",
                SITE_ID));

            if (Loader::includeModule('kit.auth') && Option::get(KitAuth::idModule, 'CONFIRM_BUYER', 'N',
