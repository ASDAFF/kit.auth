    ...
    public function changeCompanyAction($companyID = false)
    {
        ...
        if (array_key_exists($companyID, $this->currentCompany)) {
            ...
            return [
                'error' => true,
                'companyId' => $companyID,
                'errorMessage' => Loc::getMessage('KIT_AUTH_COMPANY_CHOOSE_ERROR_MESSAGE_COMPANY_NOT_FOUND')
            ];
        }
        else{
            $USER->Logout();
            return [
                'error' => false,
                'errorMessage' => Loc::getMessage('KIT_AUTH_COMPANY_CHOOSE_ERROR_MESSAGE_NO_RELATED_COMPANIES')
            ];
        }
