...
// all done
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["REGISTER"] <> '') {
            ...
            } else // if there's no any errors - create user
            {
                ...
                if (intval($ID) > 0) {
                    ...
                    return [
                        'error' => false,
                        'successMessage' => GetMessage("KIT_AUTH_STAFF_REGISTER_SUCCESS", ["#LOGIN#" =>$this->arResult['VALUES']["LOGIN"]])
                    ];
                } else {
    ...
    public function confirmStaffAction()
    {
        ...
        $staffObject = \Kit\Auth\Internals\StaffTable::getList(array(
        ...
        if ($staffArray = $staffObject->fetch()) {
            ...
            if (is_numeric($result)) {
                return [
                    'error' => false,
                    'successMessage' => GetMessage("KIT_AUTH_STAFF_REGISTER_SUCCESS", ["#LOGIN#" => $arrayUser["LOGIN"]])
                ];
            } else {
                return [
                    'error' => true,
            ...
            if(isset($email) && check_email($email)){
                ...
                if($existUser['error'] === true){
                    ...
                    if (intval($ID) > 0){
                        ...
                        return [
                            'error' => false,
                            'successMessage' => GetMessage("KIT_AUTH_STAFF_REGISTER_REFERRAL_SUCCESS")
                        ];
                    }
                    else{
                        return [
                ...
                return [
                    'error' => true,
                    'errorMessage' => GetMessage("KIT_AUTH_STAFF_REGISTER_ERROR_EMAIL")
                ];
            }
        }
    }
