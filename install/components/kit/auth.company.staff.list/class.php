...
class CompanyStaffList extends CBitrixComponent implements Controllerable
{
    ...
    private function setFieldsUser($staff){
            ...
            if($staff["STATUS"] == "Y" && $this->isAdmin && (!$_SESSION["SHOW_ALL_USERS"] || $_SESSION["SHOW_ALL_USERS"] == "N")){
                $this->arResult["CONFIRM_".$staff["STATUS"]]['ROWS'][$staff["ID"]]['actions'][0]["TEXT"] = GetMessage("KIT_AUTH_COMPANY_STAFF_LIST_ACTION_REMOVE");
            ...
            elseif($staff["STATUS"] == "N"){
                $this->arResult["CONFIRM_".$staff["STATUS"]]['ROWS'][$staff["ID"]]['actions'][0]["TEXT"] = GetMessage("KIT_AUTH_COMPANY_STAFF_LIST_ACCEPT");
                $this->arResult["CONFIRM_".$staff["STATUS"]]['ROWS'][$staff["ID"]]['actions'][0]["ONCLICK"] = "confirmUser(".$staff["ID"].", ".$staff["COMPANY_ID"].")";
                $this->arResult["CONFIRM_".$staff["STATUS"]]['ROWS'][$staff["ID"]]['actions'][0]["DEFAULT"] = "1";
                $this->arResult["CONFIRM_".$staff["STATUS"]]['ROWS'][$staff["ID"]]['actions'][1]["TEXT"] = GetMessage("KIT_AUTH_COMPANY_STAFF_LIST_UNACCEPT");
    ...
    private function showStaffList($filter = [], $status)
    {
        ...
        $this->arResult["NAV_STRING_STAFF_".$status] = $rs->GetPageNavString(GetMessage("KIT_AUTH_COMPANY_STAFF_LIST_PAGE_NAV_STRING"));
    ...
    public function removeUserCompanyAction($userTableId, $companyId)
    {
        ...
        if($userTableId && $companyId){
            ...
            if($company->isUserAdmin($USER->getID(), $companyId)){
                ...
                return [
                    'error' => true,
                    'errorMessage' => GetMessage("KIT_AUTH_COMPANY_CHOOSE_ERROR_MESSAGE_PERMISSION_DENIED")
                ];
            }
        }
        else{
            return [
                'error' => true,
                'errorMessage' => GetMessage("KIT_AUTH_COMPANY_CHOOSE_ERROR_MESSAGE_NO_RELATED_COMPANIES")
            ];
        }
    }
