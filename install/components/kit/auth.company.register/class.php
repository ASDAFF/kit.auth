...
use Bitrix\Main\Config\Option,
    ...
    Kit\Auth\Internals\BuyerConfirmTable,
    Kit\Auth\Internals\CompanyConfirmTable,
    Kit\Auth\Internals\UserConfirmTable,
    Kit\Auth\User\WholeSaler;
...
class CompanyRegister extends CBitrixComponent implements Controllerable
{
    ...
    private function setDefaultParams()
    {
        ...
        $this->arParams['LOGIN_EQ_EMAIL'] = Option::get("kit.auth", "LOGIN_EQ_EMAIL", "N", SITE_ID);
...
// time zones
    ...
    {
        $requiredFields = unserialize(Option::get(KitAuth::idModule, 'GROUP_REQUIRED_FIELDS_' . $this->whosalerId, '', SITE_ID));
    ...
    {
        $orderOptFields = unserialize(Option::get(KitAuth::idModule, 'GROUP_ORDER_FIELDS_' . $this->whosalerId, '', SITE_ID));
    ...
    private function registerCompany($request)
    {
        ...
        if($this->eventResult) {
            ...
            if ($request->get("REGISTER_WHOLESALER")) {
                ...
                if ($confirmRegister == 'Y') {
                    ...
                    if($this->issetCompany && is_numeric($this->issetCompany["ID"]) &&  $request->get('CONFIRM_JOIN')=="Y"){
                        $confirmRoles = unserialize(Option::get('kit.auth', 'COMPANY_STAFF_ROLE', 'ADMIN'));
            ...
            if (intval($ID) > 0) {
                ...
                if (Option::get('kit.auth', 'CONFIRM_BUYER', 'N') == 'Y' && !$this->issetCompany["ID"]) {
                    $moderationBuyer = true;
                    $this->registerValues["ORDER_FIELDS"]["STATUS"] = "M";

                }

                $company = new \Kit\Auth\Company\Company(SITE_ID);
