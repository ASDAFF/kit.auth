<?php
use Kit\Seometa\SeometaStatisticsTable;
...
$APPLICATION->AddHeadScript("/bitrix/js/kit.seometa/script.js");
...
$POST_RIGHT = $APPLICATION->GetGroupRight("kit.auth");
if($POST_RIGHT=="D")
...
$APPLICATION->SetTitle(GetMessage("KIT_AUTH_TITLE"));
...
?>
    <form name="find_form" method="get" action="<?echo $APPLICATION->GetCurPage();?>">
        ...
        <tr>
            <td>
                <?=GetMessage("KIT_AUTH_EVENT_NAME")?>:
            </td>
            <td>
                <?php
                $sources = unserialize(Option::get("kit.auth", 'AUTH_FROM_EMAIL_EVENTS','a:0:{}','s1'));
                $so = array(
                    'reference'=>array('-'),
                    'reference_id'=>array(''),
                );
        ...
        <tr>
            <td><?echo GetMessage("KIT_AUTH_TIME")." (".FORMAT_DATE."):"?></td>
...
<?
$open_mails = Kit\Auth\Internals\StatisticsTable::getOpenMails($arFilter);
$return_mails = Kit\Auth\Internals\StatisticsTable::getReturnMails($arFilter);
?>
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var legendOrder;
            var selectedOrder;
            var chartDataOpen =[{"percent":<?=$open_mails['Y']?>,"type":"<?=GetMessage('KIT_AUTH_OPEN_MAIL')?>","color":"#f3a882"},
                {"percent":<?=$open_mails['N']?>,"type":"<?=GetMessage('KIT_AUTH_NO_OPEN_MAIL')?>","color":"#f2cb78"}
                ];
            var chartDataReturn =[{"percent":<?=$return_mails['Y']?>,"type":"<?=GetMessage('KIT_AUTH_RETURN_MAIL')?>","color":"#00ff00"},
                {"percent":<?=$return_mails['N']?>,"type":"<?=GetMessage('KIT_AUTH_NO_RETURN_MAIL')?>","color":"#ff0000"}
                ];
// create chart
    ...
    <h3><?=GetMessage('KIT_AUTH_OPEN')?></h3>
    <div id="chartdivorder" class="charts"></div>
    <h3><?=GetMessage('KIT_AUTH_RETURN')?></h3>
