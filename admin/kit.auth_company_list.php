...
Loader::includeModule(KitAuth::idModule);
if(!Loader::includeModule("sale")) {
    return false;
}
Loc::loadMessages(__FILE__);

$postRight = $APPLICATION->GetGroupRight("kit.auth");
if($postRight == "D") {
...
$arrHeaders = [
    [
        "id"      => "ID",
        "content" => Loc::getMessage(KitAuth::idModule."_ID"),
        "sort"    => "ID",
        "align"   => "right",
        "default" => true,
    ],
    [
        "id"      => "NAME",
        "content" => Loc::getMessage(KitAuth::idModule."_TITLE"),
        "sort"    => "NAME",
        "default" => true,
    ],
    [
        "id"      => "INN",
        "content" => Loc::getMessage(KitAuth::idModule."_INN"),
        "sort"    => "INN",
        "default" => true,
    ],
    [
        "id"      => "BUYER_TYPE",
        "content" => Loc::getMessage(KitAuth::idModule."_BUYER_TYPE"),
        "sort"    => "BUYER_TYPE",
        "default" => false,
    ],
    [
        "id"      => "STAFF",
        "content" => Loc::getMessage(KitAuth::idModule."_STAFF"),
        "sort"    => "ACTIVE",
        "align"   => "left",
        "default" => true,
    ],
    [
        "id"      => "DATE_CREATE",
        "content" => Loc::getMessage(KitAuth::idModule."_DATE_CREATE"),
        "sort"    => "DATE_CREATE",
        "default" => true,
    ],
    [
        "id"      => "DATE_UPDATE",
        "content" => Loc::getMessage(KitAuth::idModule."_DATE_CHANGE"),
        "sort"    => "DATE_UPDATE",
        "default" => true,
    ],
    [
        "id"      => "ACTIVE",
        "content" => Loc::getMessage(KitAuth::idModule."_ACTIVE"),
        "sort"    => "ACTIVE",
        "default" => true,
    ],
    [
        "id"      => "STATUS",
        "content" => Loc::getMessage(KitAuth::idModule."_STATUS"),
        "sort"    => "STATUS",
        "default" => true,
    ],
];
                ...
                try {
                    ...
                    if(!$result->isSuccess()) {
                        $lAdmin->AddGroupError(
                            Loc::getMessage(KitAuth::idModule."_SAVE_ERROR")." ".Loc::getMessage(
                                KitAuth::idModule."_NO_ZAPIS"
                            ),
                            $ID
                        );
                    }
                    ...
                    $lAdmin->AddGroupError(
                        Loc::getMessage(KitAuth::idModule."_SAVE_ERROR")." ".$e->getMessage(),
                        $ID
                    );
                }
        } else {
    ...
    foreach($arID as $ID) {
        ...
        switch($_REQUEST['action']) {
            case "delete":
                ...
                if(!$result->isSuccess()) {
                    $lAdmin->AddGroupError(
                        Loc::getMessage(KitAuth::idModule."_DEL_ERROR")." ".Loc::getMessage(
                            KitAuth::idModule."_NO_ZAPIS"
                        ),
                        $ID
                    );
...
$rsData->NavStart();
$lAdmin->NavText($rsData->GetNavPrint(Loc::getMessage(KitAuth::idModule."_NAV")));
...
while($arRes = $rsData->NavNext(true, "f_")):
    ...
    if(!empty($arStaff[$arRes['ID']])) {
        foreach($arStaff[$arRes['ID']] as $staff) {
            $staffs .=
                '[<a href="/bitrix/admin/user_edit.php?lang='.LANG.'&ID='.$staff['USER_ID'].'">'.Loc::getMessage(KitAuth::idModule.'_USER').'</a>] '.
    ...
    if(!empty($arRes['STATUS'])) {
        $status = Loc::getMessage(KitAuth::idModule."_STATUS_".$arRes['STATUS']);
    }

    $active = '';
    if(!empty($arRes['ACTIVE'])) {
        $active = Loc::getMessage(KitAuth::idModule."_ACTIVE_".$arRes['ACTIVE']);
    }
    ...
    // in edit mode
    $row->AddViewField("NAME", '<a href="kit.auth_company_edit.php?ID='.$f_ID.'&lang='.LANG.'">'.$f_NAME.'</a>');
    ...
    /*$arActions[] = [
        "ICON"    => "edit",
        "DEFAULT" => true,
        "TEXT"    => Loc::getMessage(KitAuth::idModule."_EDIT"),
        "ACTION"  => $lAdmin->ActionRedirect("kit.auth_company_edit.php?ID=".$f_ID),
    ];*/
    if($postRight >= "W") {
        $arActions[] = [
            "ICON"   => "delete",
            "TEXT"   => Loc::getMessage(KitAuth::idModule."_DEL"),
...
$lAdmin->AddFooter(
    [
        ["title" => Loc::getMessage(KitAuth::idModule."_LIST_SELECTED"), "value" => $rsData->SelectedRowsCount()],
        ["counter" => true, "title" => Loc::getMessage(KitAuth::idModule."_LIST_CHECKED"), "value" => "0"],
    ]
);

/*$lAdmin->AddGroupActionTable(
    [
        "delete"     => Loc::getMessage(KitAuth::idModule."_LIST_DELETE"),
        "activate"   => Loc::getMessage(KitAuth::idModule."_LIST_ACTIVATE"),
        "deactivate" => Loc::getMessage(KitAuth::idModule."_LIST_DEACTIVATE"),
    ]
);*/
...
$aContext = [
    [
        "TEXT"  => Loc::getMessage(KitAuth::idModule."_POST_ADD_TEXT"),
        "LINK"  => "kit.auth_company_edit.php?lang=".LANG,
        "TITLE" => Loc::getMessage(KitAuth::idModule."_POST_ADD_TITLE"),
        "ICON"  => "btn_new",
    ],
];
...
$oFilter = new CAdminFilter(
    $tableName."_filter",
    [
        Loc::getMessage(KitAuth::idModule."_ID"),
        Loc::getMessage(KitAuth::idModule."_NAME"),
        Loc::getMessage(KitAuth::idModule."_INN"),
        Loc::getMessage(KitAuth::idModule."_STAFF"),
        Loc::getMessage(KitAuth::idModule."_BUYER_TYPE"),
        Loc::getMessage(KitAuth::idModule."_STATUS"),
        Loc::getMessage(KitAuth::idModule."_ACTIVE"),
    ]
);

if(KitAuth::returnDemo() == 3 || KitAuth::returnDemo() == 0) {
    ?>
    <div class="adm-info-message-wrap adm-info-message-red">
        <div class="adm-info-message">
            <div class="adm-info-message-title"><?= getMessage(KitAuth::idModule."_DEMO_END") ?></div>
...
?>
    <form name="find_form" method="get" action="<? echo $APPLICATION->GetCurPage(); ?>">
        <? $oFilter->Begin(); ?>
        <!--<tr>
            <td><b><?/*= Loc::getMessage(KitAuth::idModule."_FIND") */?>:</b></td>
            <td>
                <input type="text" size="25" name="find" value="<?/* echo htmlspecialchars($find) */?>"
                       title="<?/*= Loc::getMessage(KitAuth::idModule."_FIND_TITLE") */?>">
                <?/*
                $arr = [
                    "reference"    => [
                        "ID",
        ...
        <tr>
            <td><?= Loc::getMessage(KitAuth::idModule."_ID") ?>:</td>
            <td>
                <input type="text" name="find_id" size="47" value="<? echo htmlspecialchars($find_id) ?>">
            </td>
        </tr>
        <tr>
            <td><?= Loc::getMessage(KitAuth::idModule."_NAME") ?>:</td>
            <td>
        ...
        <tr>
            <td><?= Loc::getMessage(KitAuth::idModule."_STATUS") ?>:</td>
            <td>
                <?
                $arr = [
                    "reference"    => [
                        Loc::getMessage(KitAuth::idModule."_STATUS_M"),
                        Loc::getMessage(KitAuth::idModule."_STATUS_R"),
                        Loc::getMessage(KitAuth::idModule."_STATUS_A"),
                    ],
                    "reference_id" => [
                        "M",
                        "R",
        ...
        <tr>
            <td><?= Loc::getMessage(KitAuth::idModule."_ACTIVE") ?>:</td>
            <td>
                <?
                $arr = [
                    "reference"    => [
                        Loc::getMessage(KitAuth::idModule."_ACTIVE_Y"),
                        Loc::getMessage(KitAuth::idModule."_ACTIVE_N"),
                    ],
                    "reference_id" => [
                        "Y",
                        "N"
...
if(KitAuth::returnDemo() == 2) {
    ...
    <div class="adm-info-message-wrap adm-info-message-red">
        <div class="adm-info-message">
            <div class="adm-info-message-title"><?= getMessage(KitAuth::idModule."_DEMO") ?></div>
            <div class="adm-info-message-icon"></div>
        </div>
    </div>
    <?
