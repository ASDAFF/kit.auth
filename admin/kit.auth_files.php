...
$id_module = 'kit.auth';
if( !Loader::includeModule( "kit.auth" ) || !Loader::includeModule( "sale" ))
{
	return;
}
...
if($idBuyers)
{
	...
	while($site = $rs->fetch())
	{
		$pts = unserialize(Option::get('kit.auth','WHOLESALERS_PERSON_TYPE','',$site['LID']));
		if(!is_array($pts))
		{
			$pts = [];
		}
		if($pts)
		{
			foreach($pts as $pt)
			{
				$inn = Option::get('kit.auth','GROUP_ORDER_INN_FIELD_'.$pt,'',$site['LID']);
				if($inn)
				{
