...
$aTabs = [
    [
        "DIV"   => "edit1",
        "TAB"   => Loc::getMessage(KitAuth::idModule."_EDIT_TAB_ORGANIZATION"),
        "ICON"  => "main_user_edit",
        "TITLE" => Loc::getMessage(KitAuth::idModule."_EDIT_TAB_ORGANIZATION_TITLE"),
    ],
    [
        "DIV"   => "edit2",
        "TAB"   => Loc::getMessage(KitAuth::idModule."_EDIT_TAB_STAFF"),
        "ICON"  => "main_user_edit",
        "TITLE" => Loc::getMessage(KitAuth::idModule."_EDIT_TAB_STAFF_TITLE"),
    ],
];

$tabControl = new CAdminForm("kit_auth_add_company", $aTabs);
...
if($ID > 0) {
    ...
    if(empty($company)) {
        LocalRedirect("/bitrix/admin/kit.auth_company_list.php?lang=".LANG);
    }
}
...
if($_REQUEST['action'] === 'delete' && $ID > 0) {
    ...
    if($result->isSuccess()) {
        LocalRedirect('/bitrix/admin/kit.auth_company_list.php?lang='.LANGUAGE_ID);
    }
}
...
if($REQUEST_METHOD === "POST" && ($save != "" || $apply != "") && $POST_RIGHT === "W" && check_bitrix_sessid()) {
    ...
    if($res) {
        if($apply != "") {
            LocalRedirect(
                "/bitrix/admin/kit.auth_company_edit.php?ID=".$ID."&mess=ok&lang=".LANG."&"
...
$APPLICATION->SetTitle(
    ($ID > 0
        ? Loc::getMessage(KitAuth::idModule."_EDIT_EDIT").$ID
        : Loc::getMessage(
            KitAuth::idModule."_EDIT_ADD"
        ))
);
...
if(KitAuth::returnDemo() == 2) {
    ...
    <?
} elseif(KitAuth::returnDemo() == 3 || KitAuth::returnDemo() == 0) {
    ?>
    <div class="adm-info-message-wrap adm-info-message-red">
        <div class="adm-info-message">
...
// back
$aMenu[] = [
    "TEXT"  => Loc::getMessage(KitAuth::idModule."_EDIT_BACK"),
    "TITLE" => Loc::getMessage(KitAuth::idModule."_EDIT_BACK_TITLE"),
    "ICON"  => "btn_list",
    "LINK"  => "kit.auth_company_list.php?lang=".LANG,
];
if($ID > 0) {
    $aMenu[] = [
        "SEPARATOR" => "Y",
    ...
    // del
    $aMenu[] = [
        "TEXT"  => Loc::getMessage(KitAuth::idModule."_EDIT_DEL"),
        "TITLE" => Loc::getMessage(KitAuth::idModule."_EDIT_DEL_TITLE"),
        "LINK"  => "javascript:if(confirm('".Loc::getMessage(KitAuth::idModule."_EDIT_DEL_CONF")
            ."'))window.location='kit.auth_company_edit.php?ID=".$ID."&action=delete&lang=".LANG."&"
            .bitrix_sessid_get()."';",
        "ICON"  => "btn_delete",
    ];
}
...
if(empty($arrBuyerTypes)) {
    CAdminMessage::ShowMessage(["MESSAGE" => Loc::getMessage(KitAuth::idModule."_EMPTY_BUYER_TYPE"),]
    );
}
if(empty($arSaleOrderProps)) {
    CAdminMessage::ShowMessage(["MESSAGE" => Loc::getMessage(KitAuth::idModule."_EMPTY_SALE_ORDER_PROPS"),]
    );
}
...
if($_REQUEST["mess"] === "ok" && $ID > 0) {
    CAdminMessage::ShowMessage(
        [
            "MESSAGE" => Loc::getMessage(KitAuth::idModule."_EDIT_SAVED"),
            "TYPE"    => "OK",
        ]
    );
}
...
// Tab1
if($ID) {
    $tabControl->AddViewField('ID', Loc::getMessage(KitAuth::idModule."_EDIT_ID"), $ID, false);
    $tabControl->AddViewField(
        'DATE_CREATE',
        Loc::getMessage(KitAuth::idModule."_EDIT_DATE_CREATE"),
        $company['DATE_CREATE'],
        false
    );
    ...
    $tabControl->AddViewField(
        'DATE_UPDATE',
        Loc::getMessage(KitAuth::idModule."_EDIT_DATE_CHANGE"),
        $company['DATE_UPDATE'],
        false
    );
}
$tabControl->AddCheckBoxField(
    "ACTIVE",
    Loc::getMessage(KitAuth::idModule."_EDIT_ACT"),
    false,
    "Y",
...
$tabControl->AddEditField(
    "NAME",
    Loc::getMessage(KitAuth::idModule."_EDIT_NAME"),
    true,
...
// Buyer types
$tabControl->BeginCustomField("BUYER_TYPE", Loc::getMessage(KitAuth::idModule."_EDIT_BUYER_TYPE"), true); ?>
    <tr id="tr_BUYER_TYPE">
...
//status
$tabControl->BeginCustomField("STATUS", Loc::getMessage(KitAuth::idModule."_EDIT_STATUS"), true); ?>
    <tr id="tr_STATUS">
        <td><?=$tabControl->GetCustomLabelHTML()?></td>
        <td>
            <?
                $arr = [
                    "reference" => [
                        Loc::getMessage(KitAuth::idModule."_EDIT_STATUS_A"),
                        Loc::getMessage(KitAuth::idModule."_EDIT_STATUS_R"),
                        Loc::getMessage(KitAuth::idModule."_EDIT_STATUS_M"),
...
// Tab2
$tabControl->BeginCustomField("STAFF", Loc::getMessage(KitAuth::idModule."_EDIT_STAFF"), false);
if($ID) {

    $hrefStaffAdd = CUtil::JSEscape($APPLICATION->GetCurPageParam("", ["ADD_STAFF"]));
    $hrefStaffAdd .= (stripos($hrefStaffAdd, '?') !== false ? '&' : '?').'ADD_STAFF=';

    CAdminMessage::ShowNote(Loc::getMessage(KitAuth::idModule."_ADD_STAFF_SAVE_INFO"));
    ...
    <script>
        ...
        function ChSTAFF()
        {
            if (tvSTAFF!=document.kit_auth_add_company_form['STAFF'].value)
            {
                tvSTAFF=document.kit_auth_add_company_form['STAFF'].value;
                if (tvSTAFF)
                {
    ...
    CAdminMessage::ShowMessage(
        ["MESSAGE" => Loc::getMessage(KitAuth::idModule."_ADD_STAFF_ERROR_ID"),]
    );
}
...
$tabControl->Buttons([
     'disabled' => false,
     'back_url' => 'kit.auth_company_list.php?lang='.LANGUAGE_ID,
 ]);
