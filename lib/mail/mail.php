...
class Mail extends \KitAuth
{
	...
	{
		$this->user = new \Kit\Auth\User\User();
	}
	...
	public function replaceLinks()
	{
		...
		$rs = \Kit\Auth\Internals\MessageTable::add(
				array(
						'EVENT_NAME' => $this->getField( 'EVENT_NAME' ),
				...
				foreach( $find[2] as $i => $link )
				{
					...
					if( strpos( $link, $this->getField( 'SITE_URL' ) ) !== false )
					{
						if( strpos( $link, '?' ) !== false )
						{
							...
							{
								$addstr .= '&KIT_AUTH_MESSAGE=' . $this->getField( 'ID' ).'&KIT_AUTH_HASH=' . $this->getField( 'HASH' );
							}
						}
						else 
						{
	...
	public function addStatistics()
	{
        ...
        $mes = \Kit\Auth\Internals\StatisticsTable::add($insert);
        if( $mes->isSuccess() )
        {
            $message = $this->getField('NEW_TEXT');
            $message .= '<img src="'.$this->getField( 'SITE_URL' ).'/bitrix/admin/kit_auth_img.php?OPEN_MESSAGE='.$mes->getId().'" width="1px" height="1px"  />';
            $this->setField('NEW_TEXT', $message);
        }
		}
	}
