	...
	public function OnPrologHandler()
	{
		...
		if(\Bitrix\Main\Loader::includeModule("kit.auth"))
		{
			...
			if($KitAuth->getDemo())
			{
				...
				$hash = $request->get("KIT_AUTH_HASH");
				$idMessage = $request->get("KIT_AUTH_MESSAGE");
				if($hash && $idMessage > 0)
				{
					$Message = \Kit\Auth\Internals\MessageTable::getList(
						[
							'select' => [
								'ID',
								'ID_USER'
					...
					{
						$data = [];
						$mes = \Kit\Auth\Internals\StatisticsTable::getList(['filter' => ['ID_MESSAGE' => $Message['ID']]])->fetch();
						...
						\Kit\Auth\Internals\StatisticsTable::update($mes['ID'], $data);
						if(!$USER->IsAuthorized())
						{
							$USER->Authorize($Message['ID_USER']);
							\Kit\Auth\Internals\MessageTable::update(
								$Message['ID'],
								[
