...
class WholeSaler extends User
{
	...
	const SendWholeSalerNotice = 'KIT_AUTH_WHOLESALER_NOTICE';

	public function __construct($site = '')
	{
		$this->_user = new User($site);
		$this->_group = Option::get(\KitAuth::idModule, 'WHOLESALERS_GROUP', '', $this->_user->getSite());
		$this->_personType = unserialize(Option::get(\KitAuth::idModule, 'WHOLESALERS_PERSON_TYPE', '', $this->_user->getSite()));
		$this->_emailNotification = Option::get(\KitAuth::idModule, 'WHOLESALERS_EMAIL_NOTIFICATION', '', $this->_user->getSite());
		$this->_linkOrderFields = unserialize(Option::get(\KitAuth::idModule, 'WHOLESALERS_ORDER_FIELDS', '', $this->_user->getSite()));
	...
	public function getInns()
	{
		...
		while ($profile = $res->Fetch())
		{
			...
			$code = Option::get(\KitAuth::idModule, 'GROUP_ORDER_INN_FIELD_' . $profile['PERSON_TYPE_ID'], '', $Site['LID']);
