...
class UserConfirmTable extends \DataManagerEx_Auth
{
    ...
    public static function OnAfterAdd(Entity\Event $event)
    {
        ...
        Main\Mail\Event::send(
            [
                'EVENT_NAME' => 'KIT_AUTH_CONFIRM_REGISTRATION',
                'LID' => $fields['LID'],
                'C_FIELDS' => array(
    ...
    public static function OnBeforeUpdate(Entity\Event $event)
    {
        ...
        if($fields['STATUS'])
        {
            ...
            if(Option::get( \KitAuth::idModule, "EXTENDED_VERSION_COMPANIES", "N") == "Y"){
                ...
                if(!($userUpdate = $user->Update($row["ID_USER"], ["ACTIVE" => "Y"]))){
                    ...
                    $rsEvents = GetModuleEvents(\KitAuth::idModule, "OnBeforeAdminModerate");
            ...
            if($idUser > 0)
            {
                ...
                $confirmBuyer = Option::get('kit.auth', 'CONFIRM_BUYER', 'N', $fields['LID']);
                if($confirmBuyer == 'N') {
                    if($row['FIELDS']['ORDER_FIELDS'])
                    {
        ...
        elseif (!$fields['STATUS']){
            if(Option::get( \KitAuth::idModule, "EXTENDED_VERSION_COMPANIES", "N") == "Y"){
                ...
                $rsEvents = GetModuleEvents(\KitAuth::idModule, "OnBeforeAdminRejected");
                while ($arEvent = $rsEvents->Fetch())
                {
                    ExecuteModuleEvent($arEvent, $row["FIELDS"]);
                }
