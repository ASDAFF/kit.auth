...
class StaffTable extends \DataManagerEx_Auth
{
    ...
    public static function OnBeforeUpdate(Entity\Event $event)
    {
        ...
        if($fields['STATUS'] && $fields['STATUS'] == "Y")
        {
            ...
            $rsEvents = GetModuleEvents(\KitAuth::idModule, "OnBeforeStaffUpdate");
    ...
    public static function OnBeforeDelete(Entity\Event $event)
    {
        ...
        $rsEvents = GetModuleEvents(\KitAuth::idModule, $messageId);
        while ($arEvent = $rsEvents->Fetch())
        {
    ...
    public static function OnBeforeAdd(Entity\Event $event)
    {
        ...
        if($fields && $arParams["fields"]["STATUS"] == "Y"){
            ...
            $rsEvents = GetModuleEvents(\KitAuth::idModule, "OnBeforeStaffInvite");
            while ($arEvent = $rsEvents->Fetch())
            {
        ...
        elseif ($fields && $arParams["fields"]["STATUS"] == "N"){
            $rsEvents = GetModuleEvents(\KitAuth::idModule, "OnBeforeJoinRequest");
            while ($arEvent = $rsEvents->Fetch())
            {
                ExecuteModuleEvent($arEvent, $fields);
            }
