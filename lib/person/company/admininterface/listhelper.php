...
//				$row->AddActions($this->getRowActions($data));
		...
		$Moderation = [
			"approve" => Loc::getMessage("KIT_AUTH_APPROVE"),
			"unapprove" => Loc::getMessage("KIT_AUTH_UNAPPROVE")
		];
	...
	)
	{
		...
		{
            $company = new Company();
            $resConfirm = $company->confirmCompany($id);
            if($resConfirm["error"]){
                $this->addErrors($resConfirm["message"]);
            }
		}
	...
	)
	{
		...
		if($action == 'unapprove' || $action == 'delete')
		{
			foreach ($IDs as $id)
			{
                $company = new Company();
