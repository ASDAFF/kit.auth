...
namespace Kit\Auth\Person\Company\AdminInterface;

use Bitrix\Main\Localization\Loc;
use DigitalWand\AdminHelper\Helper\AdminEditHelper;
use DigitalWand\AdminHelper\Helper\AdminListHelper;
use Kit\Auth\Internals\CompanyConfirmTable;

Loc::loadMessages(__FILE__);

class EditHelper extends AdminEditHelper
{
	protected static $model = '\Kit\Auth\Internals\CompanyConfirmTable';
	...
	private function showTabElements($tabSettings)
	{
		...
		if($this->data['FIELDS']) ;
		{
			$this->tabControl->BeginCustomField('NAME', Loc::getMessage('KIT_AUTH_EDIT_NAME'));
			echo '<tr>';
			print '<td width="40%" style="vertical-align: top;">' . Loc::getMessage('KIT_AUTH_EDIT_NAME') . ':</td>';
			echo '<td width="60%">' . $this->data['FIELDS']['NAME'] . '</td>';
			echo '</tr>';
			$this->tabControl->EndCustomField('NAME');
			$this->tabControl->BeginCustomField('LAST_NAME', Loc::getMessage('KIT_AUTH_EDIT_LAST_NAME'));
			echo '<tr>';
			print '<td width="40%" style="vertical-align: top;">' . Loc::getMessage('KIT_AUTH_EDIT_LAST_NAME') . ':</td>';
	...
	protected function customActions($action, $id = null)
	{
		...
		if($action == 'confirm')
		{
			$result = CompanyConfirmTable::update($id,['STATUS' => true]);
			if(!$result->isSuccess()){
				$this->addErrors($result->getErrorMessages());
			}
			else{
                $company = new Company();
                $company->confirmCompany($id);
            }
		}
		if($action == 'unconfirm')
		{
			$result = CompanyConfirmTable::delete($id);
			if(!$result->isSuccess()){
				$this->addErrors($result->getErrorMessages());
			}
			else{
                $company->unconfirmCompany($id);
                CompanyConfirmTable::delete($id);
            }
		}
	}
}