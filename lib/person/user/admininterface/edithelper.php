...
namespace Kit\Auth\Person\User\AdminInterface;
...
{
	protected static $model = '\Kit\Auth\Internals\UserConfirmTable';
	...
	protected function customActions($action, $id = null)
	{
		...
		if($action == 'confirm')
		{
			$result = UserConfirmTable::update($id,['STATUS' => true]);
			if(!$result->isSuccess()){
				$this->addErrors($result->getErrorMessages());
			}
		}
		if($action == 'unconfirm')
		{
			$result = UserConfirmTable::delete($id);
			if(!$result->isSuccess()){
				$this->addErrors($result->getErrorMessages());
			}
		}
