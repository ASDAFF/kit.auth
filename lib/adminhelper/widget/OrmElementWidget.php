...
/**
 * ¬иджет выбора записей из ORM.
 *
 * Ќастройки:
 * - `HELPER` Ч (string) класс хелпера, из которого будет производитьс€ поиск записией. ƒолжен быть
 * наследником `\DigitalWand\AdminHelper\Helper\AdminBaseHelper`.
 * - `ADDITIONAL_URL_PARAMS` Ч (array) дополнительные параметры дл€ URL с попапом выбора записи.
 * - `TEMPLATE` Ч (string) шаблон отображени€ виджета, может принимать значени€ select и radio, по-умолчанию Ч select.
 * - `INPUT_SIZE` Ч (int) значение атрибута size дл€ input.
 * - `WINDOW_WIDTH` Ч (int) значение width дл€ всплывающего окна выбора элемента.
 * - `WINDOW_HEIGHT` Ч (int) значение height дл€ всплывающего окна выбора элемента.
 * - `TITLE_FIELD_NAME` Ч (string) название пол€, из которого выводить им€ элемента.
 *
...
class OrmElementWidget extends NumberWidget
{
    ...
    /**
     * √енерирует HTML с выбором элемента во вcплывающем окне, шаблон select.
     *
     * @return string
     */
    ...
    /**
     * √енерирует HTML с выбором элемента в виде радио инпутов.
     *
     * @return string
     */
    ...
    /**
     * ѕолучает информацию о запис€х, к которым осуществлена прив€зка.
     *
     * @return array
    ...
    /**
     * ѕолучает информацию о всех активных элементах дл€ их выбора в виджете.
     *
     * @return array
     *
     * @throws \Bitrix\Main\ArgumentException
