...
class EventHandlers
{
	...
	{
		if( \Bitrix\Main\Loader::includeModule( "kit.auth" ) && $arTemplate['EVENT_NAME'] != \KitAuth::KIT_AUTH_SEND_EVENT)
		{
			$sended = \Kit\Auth\Mail\EventHandlers::OnBeforeEventSendHandler( $arFields, $arTemplate);
			if($sended)
			{
				$arFields = array();
				return false;
